...
class Indexer(val indexDir: File) extends Logging {
    ...
    private val conf = ConfigFactory.load("configuration.conf")
    logger.info("Configuration content: "+conf.getObject("lookup").toString())
    ...
    private def updateDataForUri(currentUri: String, fieldCollector:scala.collection.mutable.HashMap[String, scala.collection.mutable.HashSet[String]]) {
        ...
        val doc =
            if (hits.scoreDocs.length == 1) {
                ...
                val label = WikiUtil.wikiDecode(uriTerm.text.replace("http://dbpedia.org/resource/", "").replace("http://it.dbpedia.org/resource/", "").replace(conf.getString("lookup.replace.label"), ""))
...
object Indexer extends Logging {
    ...
    private def getRedirectUris(redirectsFile: File): scala.collection.Set[String] = {
        ...
        while (parser.hasNext) {
            ...
            if(triple(1).toString != conf.getString("lookup.nTriple.redirect.baseUri")) {
                throw new Exception("predicate must be "+conf.getString("lookup.nTriple.redirect.baseUri")+"; got "+triple(1).toString)
            }
